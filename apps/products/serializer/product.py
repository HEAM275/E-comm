...
class ProductCreateSerializer(AuditableSerializerMixin):
    ...
    def validate_name(self, value):
        ...
        if len(value) > 255:
            raise serializers.ValidationError(_("Name cannot exceed 255 characters."))
    ...
    def validate(self, data):
        ...
        if (
            category
            and Product.objects.filter(name=name, category_product=category).exists()
        ):
            raise serializers.ValidationError(
                {
                    "name": _(
...
class ProductUpdateSerializer(AuditableSerializerMixin):
    ...
    def validate(self, data):
        ...
        if (
            category
            and Product.objects.filter(name=name)
            .exclude(pk=instance.pk)
            .filter(category_product=category)
            .exists()
        ):
            raise serializers.ValidationError(
                {
